...
в `<config>/geo/` и обновляются раз в сутки. Обход действует и в режиме proxy, и в
режиме tun. В режиме tun direct-outbound привязывается к интерфейсу, через который
достижим сервер (тот же путь, которым туннель закрепляет соединение с сервером),
поэтому обходной трафик уходит через него, а не зацикливается обратно в туннель.
